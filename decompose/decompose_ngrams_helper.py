...
Pw  = Pdist(datafile('./file_ngrams/count_1w.txt'), N, avoid_long_words)
...
P2w = Pdist(datafile('./file_ngrams/count_2w.txt'), N)
...
P3l = Pdist(datafile('./file_ngrams/count_3l.txt'))
P2l = Pdist(datafile('./file_ngrams/count_2l.txt')) ## We'll need it later
...
P1edit = Pdist(datafile('./file_ngrams/count_1edit.txt')) ## Probabilities of single edits
...
def decompose_dataset(data,decompose_func):
    global frag
    s_dict={}
    p_dict={}
    s_list=[]
    ...
    for i,line in enumerate(data):
        ...
        if s not in s_dict.keys():
            try:
                ...
                print("ngrams decompose error",s,i,len(s))
        else:
            frag=s_dict[s]
            s_list.append(frag)
            p_list.append(p_frag)
    ...
    # print(s_list[:5],p_list[:5],l_list[:5])
